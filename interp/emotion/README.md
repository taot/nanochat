...
```bash
uv run python -m interp.emotion.scripts.emotion_probes --help
```
...
- `scripts/emotion_probes.py` — extract, evaluate, inspect, and steer emotion residual directions.
- `runs/emotion_sweep_params.sh` — parameter sweep helper for probe experiments.
- `chatui/` — browser UI for emotion detection and emotion-guided responses.
...
```bash
uv run python -m interp.emotion.scripts.emotion_probes extract \
  --source sft \
  --model-tag d24 \
  --step 483 \
  --device-type cpu \
...
```bash
uv run python -m interp.emotion.scripts.emotion_probes eval-probe \
  --vectors out/emotion_probes_smoke/vectors.pt \
  --out-dir out/emotion_probes_smoke

uv run python -m interp.emotion.scripts.emotion_probes logit-lens \
  --source sft \
  --model-tag d24 \
  --step 483 \
  --device-type cpu \
...
uv run python -m interp.emotion.scripts.emotion_probes steer \
  --source sft \
  --model-tag d24 \
  --step 483 \
  --device-type cpu \
...
```bash
uv run python -m interp.emotion.scripts.emotion_probes eval-probe --center-act
```

Inspect unembed/logit-lens tokens for each direction:

```bash
uv run python -m interp.emotion.scripts.emotion_probes logit-lens --top-k 10
```

Measure causal steering on the prompt `How does he feel?` with assistant prefix `He feels`:

```bash
uv run python -m interp.emotion.scripts.emotion_probes steer --emotion happy --strength 2.0
uv run python -m interp.emotion.scripts.emotion_probes steer --emotion sad --strength 2.0
uv run python -m interp.emotion.scripts.emotion_probes steer --emotion angry --strength 2.0
```
...
- Future work: compare probe quality across layers, or extract activations from multiple layers in one run.

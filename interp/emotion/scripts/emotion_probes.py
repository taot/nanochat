...
Example:
python -m interp.emotion.scripts.emotion_probes extract --source sft --train-per-emotion 40 --test-per-emotion 20
python -m interp.emotion.scripts.emotion_probes eval-probe
python -m interp.emotion.scripts.emotion_probes logit-lens
python -m interp.emotion.scripts.emotion_probes steer --emotion happy --strength 2.0
"""
...
@torch.inference_mode()
def generate_steered(model, tokenizer, prompt_ids, gen_steps, layer_idx=None, vector=None, strength=0.0, positions="all", temperature=1.0, top_k=None):
    ctx = steer_layer(model, layer_idx, vector, strength, positions) if vector is not None else nullcontext()
    generated = []
    with ctx:
        for tok_id in model.generate(prompt_ids, max_tokens=gen_steps, temperature=temperature, top_k=top_k):
